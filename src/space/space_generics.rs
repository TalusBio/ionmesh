...
pub trait IndexedPoints<'a, const N: usize, T> {
    ...
    fn query_ndrange(&'a self, boundary: &NDBoundary<N>, reference_point: Option<&NDPoint<N>>) -> Vec<&'a T>;
}
...
pub trait NDPointConverter<T, const D: usize> {
    ...
    fn convert_to_bounds_query<'a>(&self, point: &'a NDPoint<D>) -> (NDBoundary<D>, Option<&'a NDPoint<D>>) {
        let bounds = NDBoundary::new(
            point.values.iter().map(|x| *x - 1.).collect::<Vec<_>>().try_into().unwrap(),
            point.values.iter().map(|x| *x + 1.).collect::<Vec<_>>().try_into().unwrap(),
        );
